  ...
  , orientation_(Ogre::Quaternion::IDENTITY)
{
  ...
  Ogre::MeshPtr frame_mesh = Ogre::MeshManager::getSingleton().createPlane(
    name + "FrameMesh", Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME,
    Ogre::Plane(Ogre::Vector3::UNIT_Z, 0), 1, 1, 1, 1, true, Ogre::HardwareBuffer::HBU_STATIC,
    Ogre::HardwareBuffer::HBU_STATIC);

  // Create entities
  bar_entity_ = scene_manager_->createEntity(name + "BarEntity", bar_mesh);
  frame_entity_ = scene_manager_->createEntity(name + "FrameEntity", frame_mesh);
  ...
  frame_material_ = Ogre::MaterialManager::getSingleton().create(
    name + "FrameMaterial", Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME);
...
void BatteryBarVisual::setDimensions(int width, int height)
{
  ...
  frame_node_->setScale(world_width * frame_scale, world_height * frame_scale, 1.0f);
  
  // Update the bar scale
  bar_node_->setScale(world_width, world_height, 1.0f);
...
void BatteryBarVisual::setVoltage(float voltage)
{
  ...
  bar_node_->setScale(world_width * normalized_voltage, height_ * pixel_scale, 1.0f);
  
  // Position the bar so it grows from the left
  bar_node_->setPosition(-world_width * 0.5f + (world_width * normalized_voltage * 0.5f), 0.0f, 0.1f);
...
void BatteryBarVisual::setPosition(const Ogre::Vector3& position)
{
  ...
  root_node_->setPosition(world_position);
}
